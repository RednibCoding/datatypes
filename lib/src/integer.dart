...
abstract class signed extends integer {
  ...
  Int8List toBytes(int length) {
    var byteList = Int8List(length);
    var val = value;

    for (int i = 0; i < length; i++) {
      byteList[i] = val & 0xff;
      val = (val - byteList[i]) ~/ 256;
    }

    // Reverse
    var reversed = Int8List(length);
    for (int i = byteList.length; i > 0; i--) {
      reversed[byteList.length - i] = byteList[i - 1];
    }
    return reversed;
  }
}
