...
///
/// The `ulong` type is contagious. Most operations on `ulong` will return `ulong`
///  values
class ulong extends unsigned {
  /// 63-bit Unsigned Integer
  ///
  /// `0` to `9223372036854775807`
  ulong(int value) : super(value: value.toUnsigned(63));

  /// 63-bit Unsigned Integer
  ///
  /// `0` to `9223372036854775807`
  ///
  /// Create `ulong` from another `integer` value
  ulong.integer(integer val) : super(value: val.value.toUnsigned(64));
  ...
  /// ulong(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// ulong.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  factory ulong.fromEnvironment(String name, {int defaultValue = 0}) {
    return ulong(
      int.fromEnvironment(
        name,
  ...
  static ulong? tryParse(String source) {
  ...
  /// Returns `ulong`
  @override
  ulong operator *(dynamic other) {
    if (other is integer) {
      return ulong(value * other.value);
    } else if (other is int) {
      return ulong(value * other);
    } else if (other is double) {
      return ulong((value * other).truncate());
    } else {
  ...
  /// `this`.
  @override
  ulong remainder(dynamic other) {
    if (other is integer) {
      return ulong(value.remainder(other.value));
    } else if (other is int) {
      return ulong(value.remainder(other));
    } else if (other is double) {
      return ulong((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  ulong modPow(unsigned exponent, unsigned modulus) => ulong(value.modPow(exponent.value, modulus.value));
  ...
  @override
  ulong modInverse(unsigned modulus) => ulong(value.modInverse(modulus.value));
  ...
  @override
  ulong gcd(integer other) => ulong(value.gcd(other.value));
