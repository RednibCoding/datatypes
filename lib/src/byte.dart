...
/// `-128` to `127`
///
/// The `byte` type is contagious. Most operations on `byte` will return `byte`
///  values
class byte extends signed {
  /// 8-bit Signed integer
  ///
  /// `-128` to `127`
  byte(int value) : super(value: value.toSigned(8));

  /// 8-bit Signed integer
  ///
  /// `-128` to `127`
  ///
  /// Create `byte` from another `integer` value
  byte.integer(integer val) : super(value: val.value.toSigned(8));
  ...
  /// byte(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// byte.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  factory byte.fromEnvironment(String name, {int defaultValue = 0}) {
    return byte(
      int.fromEnvironment(
        name,
  ...
  static byte? tryParse(String source) {
  ...
  /// `this`.
  @override
  byte remainder(dynamic other) {
    if (other is integer) {
      return byte(value.remainder(other.value));
    } else if (other is int) {
      return byte(value.remainder(other));
    } else if (other is double) {
      return byte((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  byte modPow(unsigned exponent, unsigned modulus) => byte(value.modPow(exponent.value, modulus.value));
  ...
  @override
  byte modInverse(unsigned modulus) => byte(value.modInverse(modulus.value));
  ...
  @override
  byte gcd(integer other) => byte(value.gcd(other.value));
