...
///
/// The `sint` type is contagious. Most operations on `sint` will return `sint`
///  values
class sint extends signed {
  /// 32-bit Signed Integer
  ///
  /// `-2147483648` to `2147483647`
  sint(int value) : super(value: value.toSigned(32));

  /// Create `sint` from another `integer` value
  ///
  /// 32-bit Signed Integer
  ///
  /// `-2147483648` to `2147483647`
  sint.integer(integer val) : super(value: val.value.toSigned(32));
  ...
  /// sint(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// sint.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  factory sint.fromEnvironment(String name, {int defaultValue = 0}) {
    return sint(
      int.fromEnvironment(
        name,
  ...
  static sint? tryParse(String source) {
  ...
  /// Returns `sint`
  @override
  sint operator *(dynamic other) {
    if (other is integer) {
      return sint(value * other.value);
    } else if (other is int) {
      return sint(value * other);
    } else if (other is double) {
      return sint((value * other).truncate());
    } else {
  ...
  /// `this`.
  @override
  sint remainder(dynamic other) {
    if (other is integer) {
      return sint(value.remainder(other.value));
    } else if (other is int) {
      return sint(value.remainder(other));
    } else if (other is double) {
      return sint((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  sint modPow(unsigned exponent, unsigned modulus) => sint(value.modPow(exponent.value, modulus.value));
  ...
  @override
  sint modInverse(unsigned modulus) => sint(value.modInverse(modulus.value));
  ...
  @override
  sint gcd(integer other) => sint(value.gcd(other.value));
