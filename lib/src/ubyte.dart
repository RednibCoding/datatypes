...
/// `0` to `255`
///
/// The `ubyte` type is contagious. Most operations on `ubyte` will return `ubyte`
///  values
class ubyte extends unsigned {
  /// 8-bit Unsigned Integer
  ///
  /// `0` to `255`
  ubyte(int value) : super(value: value.toUnsigned(8));

  /// 8-bit Unsigned Integer
  ///
  /// `0` to `255`
  ///
  /// Create `ubyte` from another `integer` value
  ubyte.integer(integer val) : super(value: val.value.toUnsigned(8));
  ...
  /// ubyte(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// ubyte.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  factory ubyte.fromEnvironment(String name, {int defaultValue = 0}) {
    return ubyte(
      int.fromEnvironment(
        name,
  ...
  static ubyte? tryParse(String source) {
  ...
  /// Returns `ubyte`
  @override
  ubyte operator *(dynamic other) {
    if (other is integer) {
      return ubyte(value * other.value);
    } else if (other is int) {
      return ubyte(value * other);
    } else if (other is double) {
      return ubyte((value * other).truncate());
    } else {
  ...
  /// `this`.
  @override
  ubyte remainder(dynamic other) {
    if (other is integer) {
      return ubyte(value.remainder(other.value));
    } else if (other is int) {
      return ubyte(value.remainder(other));
    } else if (other is double) {
      return ubyte((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  ubyte modPow(unsigned exponent, unsigned modulus) => ubyte(value.modPow(exponent.value, modulus.value));
  ...
  @override
  ubyte modInverse(unsigned modulus) => ubyte(value.modInverse(modulus.value));
  ...
  @override
  ubyte gcd(integer other) => ubyte(value.gcd(other.value));
