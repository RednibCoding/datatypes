...
/// `-32768` to `32767`
///
/// The `short` type is contagious. Most operations on `short` will return `short`
///  values
class short extends signed {
  /// 16-bit Signed integer
  ///
  /// `-32768` to `32767`
  short(int value) : super(value: value.toSigned(16));

  /// 16-bit Signed integer
  ///
  /// `-32768` to `32767`
  ///
  /// Create `short` from another `integer` value
  short.integer(integer val) : super(value: val.value.toSigned(16));
  ...
  /// short(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// short.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  factory short.fromEnvironment(String name, {int defaultValue = 0}) {
    return short(
      int.fromEnvironment(
        name,
  ...
  static short? tryParse(String source) {
  ...
  /// Returns `short`
  @override
  short operator *(dynamic other) {
    if (other is integer) {
      return short(value * other.value);
    } else if (other is int) {
      return short(value * other);
    } else if (other is double) {
      return short((value * other).truncate());
    } else {
  ...
  /// `this`.
  @override
  short remainder(dynamic other) {
    if (other is integer) {
      return short(value.remainder(other.value));
    } else if (other is int) {
      return short(value.remainder(other));
    } else if (other is double) {
      return short((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  short modPow(unsigned exponent, unsigned modulus) => short(value.modPow(exponent.value, modulus.value));
  ...
  @override
  short modInverse(unsigned modulus) => short(value.modInverse(modulus.value));
  ...
  @override
  short gcd(integer other) => short(value.gcd(other.value));
