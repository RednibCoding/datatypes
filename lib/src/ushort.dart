...
/// `0` to `65535`
///
/// The `ushort` type is contagious. Most operations on `ushort` will return `ushort`
///  values
class ushort extends unsigned {
  /// 16-bit Unsigned Integer
  ///
  /// `0` to `65535`
  ushort(int value) : super(value: value.toUnsigned(16));

  /// 16-bit Unsigned Integer
  ///
  /// `0` to `65535`
  ///
  /// Create `ushort` from another `integer` value
  ushort.integer(integer val) : super(value: val.value.toUnsigned(16));
  ...
  /// ushort(int.tryParse(const String.fromEnvironment(name, defaultValue: ""))
  ///     ?? defaultValue)
  /// ```
  /// Example:
  /// ```dart
  /// ushort.fromEnvironment("defaultPort", defaultValue: 80)
  /// ```
  factory ushort.fromEnvironment(String name, {int defaultValue = 0}) {
    return ushort(
      int.fromEnvironment(
        name,
  ...
  static ushort? tryParse(String source) {
  ...
  /// Returns `ushort`
  @override
  ushort operator *(dynamic other) {
    if (other is integer) {
      return ushort(value * other.value);
    } else if (other is int) {
      return ushort(value * other);
    } else if (other is double) {
      return ushort((value * other).truncate());
    } else {
  ...
  /// `this`.
  @override
  ushort remainder(dynamic other) {
    if (other is integer) {
      return ushort(value.remainder(other.value));
    } else if (other is int) {
      return ushort(value.remainder(other));
    } else if (other is double) {
      return ushort((value.remainder(other)).truncate());
    } else {
      throw Exception('Invalid type for operand: ${other.runtimeType}');
    }
  }
  ...
  @override
  ushort modPow(unsigned exponent, unsigned modulus) => ushort(value.modPow(exponent.value, modulus.value));
  ...
  @override
  ushort modInverse(unsigned modulus) => ushort(value.modInverse(modulus.value));
  ...
  @override
  ushort gcd(integer other) => ushort(value.gcd(other.value));
